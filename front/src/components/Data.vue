<template>
  <div v-if="this.$route.query.id != null">
    ...
    <el-button v-if="statusflag < 3" style="margin:1%; " type="primary" icon="el-icon-edit"  @click="dialogFormVisible = true">修改</el-button>
    <el-button v-if="statusflag == 3" style="margin:1%; " type="primary" icon="el-icon-collection-tag"  @click="dialogScoreVisible = true">评分</el-button>

      <el-dialog title="修改工单" :visible.sync="dialogFormVisible">
  <el-form :model="form"  label-position="right" status-icon :rules="rules" ref="form" label-width="80px" class="demo-ruleForm">
    ...
    <el-form-item>
      ...
      <el-dialog title="工单评分" :visible.sync="dialogScoreVisible">
        <el-rate
          v-model="form.score"
          show-text>
      </el-rate>
        <p></p>
  <el-form :model="form"  label-position="right" status-icon :rules="rules" ref="form" label-width="80px" class="demo-ruleForm">
    <el-form-item label="评价"  style="margin-right: 40px;"  prop="problem">
      <el-input type="textarea" v-model="form.review" :rows="10"></el-input>
    </el-form-item>
                ...
                this.$axios({
                   ...
                   // url:'http://localhost:3000/getform',
                    ...
                    if(response.data!="没有表单记录")
                    {
                        ...
                        {
                            if(this.tableDataTemp[i].status =="已完成（未评价）" || this.tableDataTemp[i].status == "全部完成")
