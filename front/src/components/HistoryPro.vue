...
<script>
    ...
    export default {
        ...
        methods:{
            handleClick(row) {
                console.log(row.id);
                this.$router.push('/homepro/DataPro?id='+row.id);
            },
            getTableData(){
                this.$axios({
                    method:'post',
                   // url:'http://localhost:3000/getformbystatus',
                    url:'http://'+window.location.hostname+':3000/getformbystatus',
            ...
            getTableData2(){
                this.$axios({
                    ...
                    if(response.data!="没有表单记录")
                    {
                        ...
                        for (let i = 0; i < this.tableData.length; i++)
                        {
                            if(this.tableData[i].status =="技术处理" )
                                this.tableData1.push(this.tableData[i]);
                            else
                                this.tableData2.push(this.tableData[i]);
                        }
