<template>
  <div v-if="this.$route.query.id != null">
    ...
    <el-button v-if="tableData[0].status == '客服转交技术处理'" style="margin:1%; " type="primary" icon="el-icon-edit"  @click="dialogFormVisible = true">接单</el-button>
    <el-button v-if="tableData[0].status == '技术处理'" style="margin:1%; " type="primary" icon="el-icon-collection-tag"  @click="dialogScoreVisible = true">处理</el-button>
    ...
    export default {
        ...
        methods:{
                        ...
                        this.$axios({
                           ...
                           // url:'http://localhost:3000/updataform',
                            ...
                            data:this.qs.stringify({    //这里是发送给后台的数据
                                ...
                                csid:vm.form.csid,
                                tsname:sessionStorage.getItem("username"),
                                tsid:vm.form.tsid,
            ...
            getTableData2(){
                this.$axios({
                    method:'post',
                    //url:'http://localhost:3000/getformbyproname',
                    url:'http://'+window.location.hostname+':3000/getformbyproname',
                    data:this.qs.stringify({    //这里是发送给后台的数据
                        tsname:sessionStorage.getItem("username")
                    })
                }).then((response) =>{//这里使用了ES6的语法
